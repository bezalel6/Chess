package ver23_multithreading.model_classes.eval_classes;

import java.util.Arrays;

import static ver23_multithreading.model_classes.Board.GamePhase.*;
import static ver23_multithreading.types.Piece.*;
...
public class Tables {
    ...
    public int[][] getTable(int type, int phase, int player) {
        return pieceTables[type].getTableByGamePhase(phase, player);
    }

    static class PieceTable {

        private final int[][][] middleGame;
        private final int[][][] endGame;

        public PieceTable(int[][] middleGame, int[][] endGame) {
            this.middleGame = init(middleGame);
            this.endGame = init(endGame);
        }

        private int[][][] init(int[][] table) {
            int[][][] ret = new int[NUM_OF_PLAYERS][][];
            ret[Player.WHITE] = reverse(table);
            ret[Player.BLACK] = table;
            return ret;
        }
        ...
        public int[][] getTableByGamePhase(int phase, int player) {
            switch (phase) {
                case MIDDLE_GAME:
                case OPENING:
                    return middleGame[player];
                case END_GAME:
