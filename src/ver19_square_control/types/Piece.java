...
public abstract class Piece {
    ...
    public static boolean addMove(Move move, Piece piece, ArrayList<Move> list, Board board) {
        ...
        if (otherPiece == null) {
//            move.setCapturing(Move.NOT_CAPTURING_HASH);
            list.add(move);
            return true;
        }
...
//            move.setCapturing(Move.NOT_CAPTURING_HASH);
            list.add(move);
            return true;
        } else if (otherPiece != null && !otherPiece.isOnMyTeam(piece) && !empty) {
            move.setCapturing(otherPiece.hashCode());
            list.add(move);
        }
        return false;
    ...
    public static boolean addMove(Location movingTo, Piece piece, ArrayList<Move> list, Board board) {
        ...
        Piece otherPiece = board.getPiece(movingTo);
        if (otherPiece == null) {
            list.add(new Move(piece.pieceLoc, movingTo, board));
            return true;
        }
        if (!otherPiece.isOnMyTeam(piece)) {
            Move move = new Move(piece.pieceLoc, movingTo, board);
            move.setCapturing(otherPiece.hashCode());
            list.add(move);
        }
        return false;
    ...
    public ArrayList<Move> getPseudoMovesList(Board board) {
        ...
        //todo o(no) ↓
        generatePseudoMoves(board).forEach(list -> {
            ret.addAll(list);
        });
        return ret;
    }
