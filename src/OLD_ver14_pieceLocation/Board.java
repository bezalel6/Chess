...
//        return win.isGameOver() ? win : loss.isGameOver() ? loss : tie;
    ...
    public BoardEval checkTie() {
        if (!isInCheck(currentPlayer)) {
            ...
            if (check50Moves()) {
                return new BoardEval(GameStatus.FIFTY_MOVES);
            }
        }
    ...
    private boolean checkRepetition() {
        if (movesList.size() > 4) {
            int index = movesList.size() - 1;
            String lastMove = movesList.get(index);
            String second = movesList.get(index - 2);
            String third = movesList.get(index - 4);
    ...
    private String str() {
        ...
        for (Piece[] row : this) {
            for (Piece piece : row) {
                ...
                ret += "|";
                if (piece != null) {
                    ret += (piece.isWhite() ? ANSI_WHITE : ANSI_BLACK);
