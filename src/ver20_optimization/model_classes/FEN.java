...
import ver20_optimization.types.Piece.Player;
...
//        }
        ...
        for (; i >= 0; i--) {
            ...
            for (Piece piece : board.getLogicMat()[i]) {
                ...
                if (piece instanceof Pawn)
                    ret.append(piece.isWhite() ? 'P' : 'p');
                else {
                    ret.append(piece.isWhite() ? piece.getAnnotation().toUpperCase(Locale.ROOT) : piece.getAnnotation().toLowerCase(Locale.ROOT));
                }
            }
            if (emptySquares != 0)
                ret.append(emptySquares);
        ...
        ret.append(" ");
        ret.append(getPlayerToMove() == Player.WHITE ? "w" : "b");
