...
import ver24_flip_board.MyError;
import ver24_flip_board.model_classes.eval_classes.Eval;
import ver24_flip_board.model_classes.eval_classes.Evaluation;
import ver24_flip_board.moves.*;
import ver24_flip_board.model_classes.pieces.*;
        ...
        for (Square[] row : logicMat)
            for (Square square : row)
                if (!square.isEmpty()) {
                    ...
                    piece.setPieceIndex(piecesLists[piece.getPieceColor()].indexOf(piece));
...
//                sort for checks vs sort for stockfish
    ...
    private void replacePiece(Piece newPiece, Piece oldPiece) {
        ...
        int player = newPiece.getPieceColor();
        ArrayList<Piece> pieces = getPlayersPieces(player);
        pieces.remove(oldPiece);
        pieces.add(newPiece);
        newPiece.setPieceIndex(pieces.indexOf(newPiece));
...
//        }
        ...
        for (Iterator<Piece> iterator = playersPieces.iterator(); iterator.hasNext(); ) {
...
//            move.setPrevBoardHash(hashBoard(pieceColor));
        ...
        if (move.isCapturing()) {
//            Piece otherPiece = getPlayersPieces(Player.getOpponent(pieceColor)).get(move.getCapturingPieceIndex());
...
//            otherPiece.setCaptured(true);
            getPlayersPieces(otherPiece.getPieceColor()).remove(otherPiece);
        }
...
//            Piece otherPiece = piecesLists[Player.getOpponent(pieceColor)].get(move.getCapturingPieceIndex());
            ...
            Piece otherPiece = createPieceFromFen(PIECES_FENS[opponent][move.getCapturingPieceType()].charAt(0), movingFrom);
            assert otherPiece != null;
            getPlayersPieces(opponent).add(otherPiece);
