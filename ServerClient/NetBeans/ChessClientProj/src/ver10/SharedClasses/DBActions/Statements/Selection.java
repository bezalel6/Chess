...
public class Selection extends SQLStatement {
    ...
    protected String createStatement() {
        ...
        String str = "%s %s\nFROM %s %s\n%s".formatted(selectPrefix, selecting, selectFrom, conditionsStr, postFix);
//        return str;
    ...
    public void join(
            ...
            Condition condition,
            Col... groupBy) {
        selectFrom += "\n%s %s ON (%s)\nGROUP BY %s\n".formatted(joinType, joinWith, condition, StrUtils.splitArr(Arrays.stream(groupBy).map(Col::colName).toArray()));
    }

    public void orderBy(Col col, @Order String order) {
        postFix += " ORDER BY " + col.label() + " " + order;
    }
