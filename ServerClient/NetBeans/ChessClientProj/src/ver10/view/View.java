...
import ver10.SharedClasses.DBActions.DBResponse;
import ver10.SharedClasses.DBActions.Graphable.Graphable;
...
import ver10.SharedClasses.ui.windows.CloseConfirmationJFrame;
import ver10.view.AuthorizedComponents.AuthorizedComponent;
import ver10.view.AuthorizedComponents.Menu;
import ver10.view.AuthorizedComponents.MenuItem;
import ver10.view.Board.BoardButton;
import ver10.view.Board.BoardPanel;
import ver10.view.Dialog.SyncableList;
import ver10.view.Graph.Graph;
import ver10.view.IconManager.IconManager;
import ver10.view.SidePanel.SidePanel;
import ver10.view.SyncedJMenus.ConnectedUsers;
import ver10.view.SyncedJMenus.OngoingGames;
import ver10.view.SyncedJMenus.SyncedJMenu;
...
//                setAlwaysOnTop(true);
    ...
    private void createMenuBar() {
        ...
        for (RequestBuilder.PreMadeRequest preMadeRequest : RequestBuilder.PreMadeRequest.values()) {
            MenuItem requestMenuItem = new MenuItem(preMadeRequest.name(), preMadeRequest.authSettings) {{
                addActionListener(l -> {
...
//        Menu menu = new Menu(RequestBuilder.PreMadeRequest.Games.name(),)
...
//        str = StrUtils.wrapInHtml(str);
    ...
    public void showDBResponse(DBResponse response, String respondingTo, String title) {
        ...
        if (response instanceof Graphable graphable)
            ...
            JLabel header = new JLabel(respondingTo);
            header.setFont(FontManager.dbResponseTableHeader);
            pnl.add(header);
            pnl.add(comp);
        }

        MyDialogs.showInfo(win, pnl, title, null);
