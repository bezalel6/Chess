...
import ver7.SharedClasses.ui.dialogs.MyDialogs;
import ver7.SharedClasses.ui.windows.CloseConfirmationJFrame;
import ver7.view.Graph.Graph;
import ver7.view.List.Synced.SyncableList;
import ver7.view.List.Synced.SyncedJMenu;
import ver7.view.SidePanel.SidePanel;
import ver7.view.board.BoardButton;
...
//                });
        ...
        SyncableList connectedUsers = new SyncedJMenu("connected", e -> {
        }, SyncedListType.CONNECTED_PLAYERS);
        listsToRegister.add(connectedUsers);
        settingsMenu.add(connectedUsers.list().navigationComp());

        SyncableList ongoingGames = new SyncedJMenu("games", e -> {
        }, SyncedListType.ONGOING_GAMES);
        listsToRegister.add(ongoingGames);
        settingsMenu.add(ongoingGames.list().navigationComp());
...
//        str = StrUtils.wrapInHtml(str);
    ...
    public void connectedToServer() {
        ...
        for (SyncableList list : listsToRegister) {
            client.getMessagesHandler().registerSyncedList(list);
        }
    }
}
