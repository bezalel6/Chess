...
public class SyncedItems extends ConcurrentHashMap<String, SyncableItem> {
    public final static SyncedItems exampleUsers1 = new SyncedItems(SyncedListType.CONNECTED_PLAYERS) {{
        for (int i = 1; i <= 5; i++) {
            add(new UserInfo("id " + i));
        }
    }};
    public final static SyncedItems exampleUsers2 = new SyncedItems(SyncedListType.CONNECTED_PLAYERS) {{
        for (int i = 1; i <= 5; i++) {
            add(new UserInfo("id " + i));
        }
    }};
