...
import ver10.Server.players.Player;
import ver10.Server.players.PlayerAI;
import ver10.Server.players.PlayerNet;
import ver10.SharedClasses.*;
import ver10.SharedClasses.SavedGames.CreatedGame;
import ver10.SharedClasses.SavedGames.GameInfo;
import ver10.SharedClasses.SavedGames.UnfinishedGame;
...
import ver10.SharedClasses.networking.MyErrors;
...
//        ShowOnScreen.showOnScreen(2, frmWin);
    ...
    private <T extends SyncableItem> SyncedItems<T> prepareListForSend(SyncedItems<T> list) {
        SyncedItems<T> ret = new SyncedItems<>(list.syncedListType);
        ret.addAll(list.stream().map(item -> (T) item.getSyncableItem()).collect(Collectors.toList()));
        return list.clean();
    }
