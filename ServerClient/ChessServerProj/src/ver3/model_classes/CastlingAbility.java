...
public class CastlingAbility {
    ...
    public static int disableCastling(Player player, Location rookLoc, int castlingAbility, Board board) {
        return disableCastling(player, getSideRelativeToKing(board, player, rookLoc), castlingAbility);
    }
    ...
    public static int getSideRelativeToKing(Board board, Player player, Location rookLoc) {
        int ret = CastlingAbility.QUEEN_SIDE;
        Location kingLoc = board.getKing(player);
        if (kingLoc.getCol() < rookLoc.getCol() * Piece.getDifference(player)) {
