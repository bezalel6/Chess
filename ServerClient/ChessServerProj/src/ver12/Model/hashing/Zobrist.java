...
import ver12.Model.MoveGenerator.GenerationSettings;
import ver12.Model.PiecesBBs;
import ver12.SharedClasses.Location;
import ver12.SharedClasses.PlayerColor;
import ver12.SharedClasses.moves.CastlingRights;
...
//        System.out.println("zobrist is faster by " + (secs2 - secs1) + "nano seconds");
    ...
    public static long hash(GenerationSettings generationSettings) {
        final long[] hash = {0};
        Arrays.stream(generationSettings.generateFor()).map(Zobrist::hashPieceType).forEach(l -> hash[0] = combineHashes(hash[0], l));

        return Zobrist.combineHashes(Zobrist.hash(generationSettings.legalize()), hash[0]);
    }
