...
import ver7.SharedClasses.SavedGames.CreatedGame;
...
import ver7.SharedClasses.ui.windows.CloseConfirmationJFrame;
import ver7.server.DB.DB;
import ver7.server.game.Game;
import ver7.server.game.GameSession;
import ver7.server.players.Player;
import ver7.server.players.PlayerAI;
...
public class Server {
    ...
    private final SyncedItems<PlayerNet> netPlayers;
    private final SyncedItems<GameSession> gameSessions;
    private final SyncedItems<CreatedGame> gamePool;
    ...
    public Server() {
        ...
        autoChatterID = 0;
        netPlayers = new SyncedItems<>(SyncedListType.CONNECTED_PLAYERS, this::syncedListUpdated);
...
//        ShowOnScreen.showOnScreen(2, frmWin);
    ...
    private SyncedItems prepareListForSend(SyncedItems list) {
        if (list.syncedListType == SyncedListType.CONNECTED_PLAYERS) {
            SyncedItems<UserInfo> userInfos = new SyncedItems<>(list.syncedListType);
            ...
            SyncedItems<GameInfo> gameInfos = new SyncedItems<>(list.syncedListType);
...
//                        disconnectPlayer();
    ...
    private void handleClient(AppSocket playerSocket) throws MyErrors {
        new Thread(() -> {
            ClientMessagesHandler clientMessagesHandler = new ClientMessagesHandler(this, playerSocket);
            playerSocket.setHandlerAndStart(clientMessagesHandler);
    ...
    public void gameSetup(Player player) throws MyErrors {
        ...
        if (gameSettings == null)
            throw new MyErrors.Disconnected();
        switch (gameSettings.getGameType()) {
            ...
            case JOIN_EXISTING -> {
                GameInfo gameInfo = gamePool.get(gameSettings.getJoiningToGameID());
                if (gameInfo == null) {
    ...
    private void startGameVsAi(Player player, GameSettings gameSettings) throws MyErrors {
    ...
    public SyncedItems<UnfinishedGame> getResumableGames(Player playerNet) throws MyErrors {
        return DB.getUnfinishedGames(playerNet.getUsername()).clean();
    }

    public PlayerNet login(AppSocket appSocket) throws MyErrors {
    ...
    private Message responseToLogin(LoginInfo loginInfo) throws MyErrors {
        if (loginInfo == null)
            return null;
    ...
    public void disconnectPlayer(Player player, String cause) {
        if (player != null) {
            ...
            List<CreatedGame> del = gamePool.values()
                    .stream()
    ...
    public void endOfGameSession(GameSession session) {
        Arrays.stream(session.getPlayers()).parallel().forEach(player -> player.disconnect("end of game session"));
