...
public enum Piece implements Serializable {
    ...
    private final static String[] PIECES_ICONS_COLORS = new String[PlayerColor.NUM_OF_PLAYERS];
    ...
    static {
        ...
        for (PlayerColor playerColor : PlayerColor.PLAYER_COLORS) {
            PIECES_ICONS_COLORS[playerColor.asInt()] = switch (playerColor) {
                case WHITE -> "\u001B[37m";
                case BLACK -> "\u001B[30m";
                case NO_PLAYER -> "";
            };
            for (PieceType pieceType : PieceType.PIECE_TYPES) {
                PIECES_ICONS[playerColor.asInt()][pieceType.asInt()] = PIECES_ICONS_COLORS[playerColor.asInt()] + pieceType.getPieceIcon() + reset;
            }
        }
